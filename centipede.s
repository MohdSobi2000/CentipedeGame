...
build_mushroom_locations:
	...
	la $a2, mushroomLocations # load the address of the mushroom array into $a1
	
build_loop:	beq $t5, $a3, build_exit # branch to build exit if $t5 == $a3
	...
	sw $t0, 0($a2)    # store back the mushroom position into $a1, the mushroomLocations array
...
build_exit:	# pop a word off the stack and move the stack pointer
	lw $ra, 0($sp)
	addi $sp, $sp, 4
	jr $ra
...
draw_flea:
	...
	lw $t0, 0($a0)			# load the fleaLocation itself in $t1
	beq $t0, -1, flea_exit		# branch to flea_exit if fleaLocation == -1
	
	#####
	addi $t1, $t0, 32		# $t5 represents the location one row below the flea
...
delay_function:
	...
	addi $a2, $zero, 15000	# initialize $a2 to 15000 as delay timer
time_loop: addi $a2, $a2, -1	# decrement $a2 by 1
	bgtz $a2, time_loop	# branch if $a2 > 0
...
update_rotation_section: 	# this code block updates the new position of the rotated segment to the memory address
	...
	move $t3, $a2		# Set $t3 to $t2's contents
	lw $t2, 0($t3)
...
arr_loop:	#iterate over the loops elements to draw each body in the centiped
	...
	#####
	bne $a0, $t6, ELSE	# branch if $a0 != $t6
...
erase_centipede:
	...
	addi $a0, $zero, 10	 # $a0 represents the loop count for length for all centipede sections
	addi $t6, $zero, 0	 # $t6 represents i = 0
	la $a1, centipedeLocation # load the address of the array into $a1
	la $a2, centipedeDirection # load the address of the array into $a2
	
clear_loop:			 # loop to draw each centipede body
	beq $t6, $a0, clear_exit # branch to clear_exit if loop counter is complete
...
draw_dart:
	...
	lw $t0, 0($a1)			# load the dart location itself in $t0
